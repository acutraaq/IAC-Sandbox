...
**What is live and working:** Terminal-native document redesign (mono nav chrome, editorial rows, `~/path` eyebrows); supporting-resource bundling (LAW + KV auto-injected into every deployment); silent failure-only status check on the review page; automatic reclamation of orphaned/expired resource groups; global submission rate limiting; automatic Azure OpenAI (Foundry) key/endpoint provisioning as native workflow parameters (`foundryApiKey`/`foundryEndpoint`) on every `logic-app` / `logic-app-storage` deployment — not a `Microsoft.Web/connections` resource, which hit a subscription-policy `Deny` on first real deploy (see `docs/superpowers/specs/2026-07-13-logic-app-foundry-connection-design.md` Revision 2); see Live Deployment section below.  
...
|----------|------|---------------|-------|
| automation | `logic-app` | Logic App (Azure OpenAI key/endpoint baked in as workflow parameters) | 1 |
| automation | `logic-app-storage` | Logic App + Storage Account (Azure OpenAI key/endpoint baked in as workflow parameters) | 2 |
