...
**What is live and working:** Terminal-native document redesign (mono nav chrome, editorial rows, `~/path` eyebrows); supporting-resource bundling (LAW + KV auto-injected into every deployment); silent failure-only status check on the review page; automatic reclamation of orphaned/expired resource groups; global submission rate limiting; automatic Azure OpenAI (Foundry) API connection provisioning on every `logic-app` / `logic-app-storage` deployment; see Live Deployment section below.  
...
|----------|------|---------------|-------|
| automation | `logic-app` | Logic App + Azure OpenAI connection | 2 |
| automation | `logic-app-storage` | Logic App + Storage Account + Azure OpenAI connection | 3 |
