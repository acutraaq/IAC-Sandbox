...
> **Version:** 2.7.0 | **Last updated:** 2026-07-13 | **Status:** Active  
...
**What is live and working:** Terminal-native document redesign (mono nav chrome, editorial rows, `~/path` eyebrows); silent failure-only status check on the review page; automatic reclamation of orphaned/expired resource groups; global submission rate limiting; automatic Azure OpenAI (Foundry) provisioning on every `logic-app` / `logic-app-storage` deployment via both an auto-created `Microsoft.Web/connections` connector (pick "Azure OpenAI" from the dropdown for any new action) and a pre-wired HTTP action already calling the model out of the box, backed by native workflow parameters (`foundryApiKey`/`foundryEndpoint`) — see `docs/superpowers/archive/specs/2026-07-13-logic-app-foundry-connection-design.md` Revision 3 for why both mechanisms ship together; see Live Deployment section below. Note: the LAW + KV supporting-resource auto-bundling built in an earlier session was later removed along with other unused ARM builders (commit `a2b7eea`) — resource groups only ever contain the resources a template/slug explicitly builds.  
**Latest session (2026-07-13):** Foundry connection design finalized at Revision 3 (connector + pre-wired HTTP action both ship, see `docs/superpowers/archive/specs/2026-07-13-logic-app-foundry-connection-design.md`); RG-name uniqueness suffix removed entirely per explicit user request for short names (e.g. `my-logic-app-rg`) — **known, accepted tradeoff:** two submissions with the same primary field value now produce the same RG name and ARM silently merges the second deployment into the first RG's resources instead of erroring (see `deriveResourceGroupName` in `web/lib/deployments/rg-name.ts`). 254 web + 108 functions tests passing.  
**2026-07-10 session:** Long-term risk audit + fixes (deep multi-agent pass, 26 confirmed findings, all 11 High now resolved). Highlights: `proxy.ts` fails closed on invalid `SESSION_SECRET` instead of crashing; custom-mode resource-type allow-list trimmed to match what `buildCustomResources` can actually build; poison-queue handler now writes a failure record for malformed messages *and* auto-deletes the orphaned RG; `GET /api/deployments/:id` checks the failure record before ever saying `"accepted"`; ARM API version bumped + PUT now has a 30s timeout; CI schema-drift check now runs unconditionally (was path-filter-gated, missed functions-only changes) and `cancel-in-progress` no longer kills an in-flight main-branch deploy; review page silently polls for failure only (no misleading progress bar — see Architecture); new daily timer function reaps resource groups past their `Expiry Date` tag (only ever touches RGs it tagged itself); new global rate limit (20 submissions/hour) on `POST /api/deployments`. See HANDOFF.md for full change list.
...
|----------|------|---------------|-------|
| automation | `logic-app` | Logic App + Azure OpenAI connection (plus a pre-wired HTTP action) | 2 |
| automation | `logic-app-storage` | Logic App + Storage Account + Azure OpenAI connection (plus a pre-wired HTTP action) | 3 |
...
│   ├── store/
│   │   └── deploymentStore.ts   # mode: "template"|null (custom mode is backend-only, no frontend store state); wizard state, submission tracking
│   ├── lib/
...
```
feat: add logic-app-storage template
fix: prevent duplicate resource types in custom-mode payloads
